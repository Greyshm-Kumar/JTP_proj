...
function displayResults(restaurants) {
    ...
    restaurants.forEach(r => {
        ...
        <div class="restaurant-card" onclick="handleRestaurantClick(${r.id})">
            <h3>${r.name}</h3>
            <p><strong>Cuisine:</strong> ${r.cuisine} ${getCountryFlag(r.cuisine)}</p>
...
function displayHomeRecommendations(restaurants) {
    ...
    html += `</div>`; // Close results-grid div
...
function getCountryFlag(cuisine) {
    const flagMap = {
        'American': 'us',
        'Brazilian': 'br',
        'Caribbean': 'jm', // Jamaica (most representative Caribbean country)
        'Chinese': 'cn',
        'French': 'fr',
        'German': 'de',
        'Greek': 'gr',
        ...
        'Korean': 'kr',
        'Lebanese': 'lb',
        'Mediterranean': 'eu', // European Union flag
        'Mexican': 'mx',
        'Moroccan': 'ma',
        'Peruvian': 'pe',
        'Spanish': 'es',
    ...
    try {
        ...
        const html = `
            <div class="restaurant-detail-card">
                ...
                <p><strong>Cuisine:</strong> ${restaurant.cuisine} ${getCountryFlag(restaurant.cuisine)}</p>
